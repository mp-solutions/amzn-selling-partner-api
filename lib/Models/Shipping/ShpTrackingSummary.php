...
/**
 * ShpTrackingSummary Class Doc Comment
 *
 * @category Class
 ...
 */
class ShpTrackingSummary implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function setStatus($status)
    {
        if (!is_null($status) && (mb_strlen($status) > 60)) {
            throw new \InvalidArgumentException('invalid length for $status when calling ShpTrackingSummary., must be smaller than or equal to 60.');
        }
        if (!is_null($status) && (mb_strlen($status) < 1)) {
            throw new \InvalidArgumentException('invalid length for $status when calling ShpTrackingSummary., must be bigger than or equal to 1.');
        }
